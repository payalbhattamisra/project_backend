...
const loginUser=asyncHandler(async(req,res)=>{
  ...
  //alternative ek hi chahiye then 
  //if(!(username || email)){
  //  throw new ApiError(400,"username or email is required")
  //}
  ...
  //user-->my server i created method use here,but User mongoDb created so that we use there findOne,findById
  ...
  .json(
    new ApiResponse(
        //status,data,message,success==>ApiResponse
       200,{
...
//logout id find then refresh token remove save update ,remove cookies
...
const refreshAccessToken=asyncHandler(async(req,res)=>{
  ...
  try {
    ...
    //decoded refresh token != user provide incoming refreshtoken
    if(incomingRefreshToken !==user?.refreshToken){
      throw new ApiError(401,"Refresh Token is expired or used")
    }
    //cookies send then refresh token generate it takes time so await use
    const options={
      httpOnly:true,
      secure:true
    }
...
//file accept -multer middlewear use,loged in then we use
